...
const STRUCTURE_SYSTEM_PROMPT_RULES = [
  ...
  'Write reasoning the way a person would describe what they heard: plain language about their actual plans or errands, naming briefly what actually ties the tasks together (a shared goal, a dependency between steps), not just the topic. Never refer to "the dump" or "the transcript" as if reasoning were describing an input variable; describe the content itself. If any task is marked standalone, say so too, in a short added clause, so the user knows before ever looking at the task list that something is headed to their Inbox separately. Keep it under 40 words, one or two plain sentences, still tight, never a paragraph.',
