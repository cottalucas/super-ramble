...
- No secrets committed. `.env.example` carries placeholder names only. Keys are
  Function secrets.
- The secret guard passes. `node scripts/check-secrets.mjs` scans tracked files
  for Firebase/Google API key literals, 32-char hex values adjacent to
  "secret"/"client_secret", and literal email addresses (placeholder convention:
  `redacted@example.com`), and runs in CI on every push and PR.
